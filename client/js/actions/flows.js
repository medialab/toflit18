...
import { uniq, forIn, debounce, keys, pick } from "lodash";
  ...
  if (name === "orders" || name == "columns") {
    ...
    if (name === "columns") {
      ...
      if (orders) {
        ...
        if (orders.length != newOrders.length) selectors.set("orders", newOrders);
      }
      // remove columns size
      const sizes = selectors.get("columnsWidth");
      if (sizes) {
        const newSizes = pick(sizes, item);
        selectors.set("columnsWidth", newSizes);
      }
    } else {
