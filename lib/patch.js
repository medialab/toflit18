...
export function rewire(upper, C1, C2, operations) {
  ...
  affectedGroups.forEach(function(affectedGroup) {
    if (alreadyDone.has(affectedGroup))
      return;
    ...
    const identicalSets = (
      ...
      union(set1, set2).length === set1.length
    );

    // Associating groups
    const allGroups = _(Array.from(groups))
      .concat(complementaryGroups)
      .uniq()
      .value();

    // TODO: take added groups into account
