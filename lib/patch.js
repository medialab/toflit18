...
export function applyOperations(classification, operations) {
  ...
  //-- Renamed groups
  const renamedGroups = renameGroup.reduce(function(acc, o) {
...
export function getAffectedGroups(operations) {
  ...
  moveItem.forEach(o => {
    if (o.from)
      affectedGroups.add(o.from);
    if (o.to)
      affectedGroups.add(o.to);
  });
...
export function rewire(upper, C1, C2, operations) {
  ...
  upper.forEach(row => {
    upperToLower[row.group] = new Set(row.items);

    row.items.forEach(item => lowerToUpper[item] = row.group);
  });

  // NOTE: Take account of already passed groups!!!
  ...
  affectedGroups.forEach(function(affectedGroup) {
    ...
    // Retrieving lower groups
    let lowerGroups = upperToLower[upperGroup];

    // Computing lower groups after update
    const lowerGroupsAfterUpdate = _(operations)
      .map(o => {
        if (o.type === 'renameGroup' || o.type === 'moveItem')
          if (lowerGroups.has(o.from))
            return o.to;
          else if (lowerGroups.has(o.to))
            return o.from;
      })
      .compact()
      .uniq()
    ...
    lowerGroups = Array.from(lowerGroups);

    // Computing lower groups before update
    const newLowerGroups = union(lowerGroups, lowerGroupsAfterUpdate);

    console.log(lowerGroups, newLowerGroups);
  });
}
