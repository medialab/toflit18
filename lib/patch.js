...
export function rewire(upper, C1, C2, operations) {
  console.log(C1.filter(r => r.item === null))
  console.log(C2.filter(r => r.item === null))
  ...
  // Dropping null groups from the equation
  delete groupsToItemsC1[null];
  delete groupsToItemsC2[null];
    ...
    allGroups.forEach(group => {
      ...
      if (identicalSets) {
        links.push({
          cluster: 'c' + cluster,
          shouldExist: true,
          beforeItems: (groupsToItemsC1[group] || []).map(i => `[${i}]`).join(' '),
          afterItems: (groupsToItemsC2[group] || []).map(i => `[${i}]`).join(' '),
