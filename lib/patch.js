...
export function rewireReport(links) {
  ...
  _(links)
    ...
    .forEach((ls, group) => {
      ...
      ls.forEach(l => {
        groupToUppers[group].uppers.push(l.upper)

        const currentSet = groupToUppers[group].items;

        if (currentSet.length && currentSet.length !== l.afterItems.length)
          groupToUppers[group].problem = true;

        groupToUppers[group].items = l.afterItems;
      });
    })
    .value();
